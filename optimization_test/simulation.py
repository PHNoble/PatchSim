...
def load_disease_params(configs,patch_df):
    params = {}
...
def load_seed_schedule(configs,params,patch_df):
    try:
...
def patchsim_step(State_Array,patch_df,params,theta,seeds,vaxs,t,stoch):
    ...
    if stoch:
        ...
        actual_vax = 0
        if t in vaxs.keys():
            tmp = [vx*params['vaxeff'] for vx in vaxs[t]]
            actual_vax = np.minimum(tmp,S[t])
        S[t] =  S[t] - actual_vax
        V[t] = V[t] + actual_vax
...
def write_epicurves(configs,patch_df,State_Array):
    ...
    for i in range(len(patch_df)):
        net_sus = S[:,i]+V[:,i]
        if configs['LoadState']=='False' and int(configs['StartDate']) > 1:
            net_sus = np.lib.pad(net_sus,(1,0),'constant',constant_values=(patch_df.pops.values[i],))
        new_exposed = np.abs(np.diff(net_sus))
